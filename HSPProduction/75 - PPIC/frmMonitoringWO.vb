...
Public Class frmMonitoringWO
    ...
    Private Sub frmMonitoringWO_Shown(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Shown
        ...
        ViewBahan.SettingID = Me.Name & "Bahan"

        SetHeaderView(View)
        SetHeaderView(ViewBahan)

        ShowData()
        View.SetColumnOrder()
        ViewBahan.SetColumnOrder()
    End Sub
    ...
    ' Tampilkan Data Sesuai Kriteria
    Private Sub ShowDataBahan()
        Dim UnitProduksi = New DaftarUnitProduksi(ActiveSession).Find(cboKodeUnit.ComboBox.SelectedValue)


        Me.Cursor = Cursors.WaitCursor
        ViewBahan.SuspendLayout()

        'Baca Data
        Dim BS As New BindingSource
        Dim DataSet As DataSet = New SAPPPIC().ReadMonitoringBahanProduksiWO(lblNomorWO.Text, UnitProduksi.KodeLokasi)

        BS.DataSource = DataSet
        BS.DataMember = "View"

        ViewBahan.RowTemplate.Height = 16
        ViewBahan.DataSource = BS

        'Format Kolom
        Dim Column As DataColumn
        For Each Column In DataSet.Tables("View").Columns
            If Column.DataType.Name.ToUpper = "DATETIME" Then
                ViewBahan.Columns(Column.ColumnName).DefaultCellStyle.Format = "dd/MM/yyyy"
                ViewBahan.Columns(Column.ColumnName).DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleCenter
                ViewBahan.Columns(Column.ColumnName).HeaderCell.Style.Alignment = DataGridViewContentAlignment.MiddleCenter
            End If

            If Column.DataType.Name.ToUpper = "DOUBLE" Or Column.DataType.Name.ToUpper = "DECIMAL" Then
                ViewBahan.Columns(Column.ColumnName).DefaultCellStyle.Format = "#,##0.00"
                ViewBahan.Columns(Column.ColumnName).DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight
                ViewBahan.Columns(Column.ColumnName).HeaderCell.Style.Alignment = DataGridViewContentAlignment.MiddleRight
            End If
        Next

        Me.Cursor = Cursors.Default
        ViewBahan.ResumeLayout()
    ...
    Private Sub View_RowEnter(sender As Object, e As DataGridViewCellEventArgs) Handles View.RowEnter
        lblNomorWO.Text = View.Item(0, e.RowIndex).Value
        lblItemBahan.Text = View.Item(3, e.RowIndex).Value
        ShowDataBahan()
    End Sub
End Class