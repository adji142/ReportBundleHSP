...
Public Class frmPengeluaranBenang
    'Data Public
            ...
            Case "btSave"
                ...
                Try
                    ...
                    For Each DR In DaftarWO.Tables("View").Rows
                        NomorWO = DR("Nomor")
                        QtyPerWO = (TotalWO * (DR("Prosentase") / 100))

                        'Posting Staging
                        '---------------------------------------------------------------------------------------------
                        Dim KodeUnitSAP = New DaftarUnitProduksi(ActiveSession).Find(cboKodeUnit.SelectedValue).KodeUnitSAP
                        SAPStaging.PostMaterialIssue(KodeUnitSAP, NomorWO, TglTransaksi, txtItemProduksi.Tag, QtyPerWO, "", txtNoTransaksi.Text, KodeLokasi)
                    Next
