...
Public Class frmDaftarPenjelasanProduksi
    Private WithEvents txtTglAwal As New DateTimePicker
    Private WithEvents txtTglAkhir As New DateTimePicker

    'Form Load
    Private Sub frmDaftarPenjelasanProduksi_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        cboUnitProduksi.ComboBox.SelectedIndex = 0
        'cboUnitProduksi.ComboBox.SelectedValue = GetSetting(enumFormID.frmGudang, enumSetting.settingKodeUnit)
    End Sub
    ...
    'Form Show
    Private Sub frmDaftarPenjelasanProduksi_Shown(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Shown
        Me.Refresh()
    ...
    'Form Closing
    Private Sub frmDaftarPenjelasanProduksi_FormClosing(sender As Object, e As FormClosingEventArgs) Handles Me.FormClosing
    ...
    Private Sub ShowData(Optional ByVal ID As String = "")
        ...
        'Baca Data
        Dim DaftarPenjelasan As New DaftarPenjelasanProduksi(ActiveSession)
        Dim BS As New BindingSource
        Dim DataSet As DataSet = DaftarPenjelasan.Read(txtTglAwal.Value.Date, txtTglAkhir.Value.Date, txtCariData.Text.Trim, cboUnitProduksi.ComboBox.SelectedValue)
    ...
    Private Sub frmDaftarPenjelasanProduksi_KeyDown(ByVal sender As Object, ByVal e As KeyEventArgs) Handles Me.KeyDown
    ...
    Private Sub Execute(ByVal ID As String)
        ...
        Select Case ID
            Case "btAdd", "btAddCTXM"
                ...
                '---------------------------------------------------------------------------------------------------------
                Dim FormAdd As New frmPenjelasanProduksi("", GroupKey)
                FormAdd.ShowDialog()
            ...
            Case "btEdit", "btEditCTXM"
                ...
                '---------------------------------------------------------------------------------------------------------
                Dim FormEdit As New frmPenjelasanProduksi(DataKey, GroupKey)
                FormEdit.ShowDialog()
            ...
            Case "btDelete", "btDeleteCTXM"
                ...
                '---------------------------------------------------------------------------------------------------------
                Dim DaftarStatus As New DaftarPenjelasanProduksi(ActiveSession)

                SendKeys.Send("{RIGHT}")
                    If MessageBox.Show("Hapus Data Baris Ini ? ", "Konfirmasi", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = DialogResult.Yes Then
                        DaftarStatus.Delete(View(0, View.CurrentCell.RowIndex).Value)
