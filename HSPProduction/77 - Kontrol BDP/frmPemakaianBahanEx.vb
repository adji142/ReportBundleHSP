...
Public Class frmPemakaianBahanEx
...
#Region "Header Section"
    ...
    Private Sub FillNomorSPK()
        Try
            ...
            DS = New DataSet
            DS = SAPWorkOrder.ReadFGWorkOrderByUnitEx(New DaftarUnitProduksi(ActiveSession).Find(cboKodeUnit.SelectedValue.ToString).KodeUnitSAP)
    ...
    Private Sub ResetData()
        ...
        '-------------------------------------------------------------------------------------------------------------------
        Try
            KodeLokasi = New DaftarUnitProduksi(ActiveSession).Find(cboKodeUnit.SelectedValue.ToString).KodeLokasi
...
Jump:
    ...
    Private Sub ReadDetailSpk()
        Dim DT As DataTable = New SAPWorkOrder().ReadRMByWO(cboNomorSpk.SelectedValue, New DaftarUnitProduksi(ActiveSession).Find(cboKodeUnit.SelectedValue.ToString).KodeUnitSAP).Tables("View")
        Dim DR As DataRow
    ...
    Private Sub Data_Change(sender As Object, e As EventArgs) Handles txtNoTransaksi.TextChanged,
        ...
        If ObjectName = "cboNomorSpk" Then
            If cboNomorSpk.SelectedIndex <> cboNomorSpk.Items.Count - 1 Then
                Try
                    Dim WO = New SAPWorkOrder().FindFGByWO(New DaftarUnitProduksi(ActiveSession).Find(cboKodeUnit.SelectedValue.ToString).KodeUnitSAP, cboNomorSpk.SelectedValue)
