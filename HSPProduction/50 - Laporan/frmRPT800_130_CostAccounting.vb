...
Public Class frmRPT800_130_CostAccounting
    ...
    Private Sub FillCombo()
        ...
        _WIP.Add(New AccountWIP With {.Kode = "ALL", .NamaWIP = "<SEMUA KELOMPOK WIP>"})
    ...
    Private Sub btRefresh_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btRefresh.Click
        ...
        'Parameter
        RPTObject.ParameterFields("DATETO").CurrentValues.AddValue(txtTanggal.Value.Date)
        RPTObject.ParameterFields("WIPACCOUNT").CurrentValues.AddValue(cboKelompokWIP.ComboBox.SelectedValue)

        'Informasi
        If cboTipeLaporan.SelectedIndex + 1 Then
            RPTObject.DataDefinition.FormulaFields("JudulLaporan").Text = "'LAPORAN WIP'"
        Else
            RPTObject.DataDefinition.FormulaFields("JudulLaporan").Text = "'LAPORAN WIP - " + UCase(cboKelompokWIP.Text) + "'"
        End If

        RPTObject.DataDefinition.FormulaFields("NamaPerusahaan").Text = "'" + UCase(ActiveSession.NamaPerusahaan) + "'"
        RPTObject.DataDefinition.FormulaFields("Periode").Text = "'S/D TANGGAL :  " + Now.ToString("dd-MM-yyyy") + "'"
