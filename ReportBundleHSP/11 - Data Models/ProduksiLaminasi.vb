...
Namespace HSP.Data
    Public Class ProduksiLaminasi
        ...
        Public Property SisaDiMesin As Double
        Public Property KodeKaryawan As String
        Public Property KetToleransi As String

    End Class

    Public Class DaftarProduksiLaminasi
        ...
        'Add
        '------------------------------------------------------------------------------------------------------------------
        Public Function Add(ByVal Data As ProduksiLaminasi) As Integer
            Dim SQL As String

            SQL = "INSERT INTO produksilaminasi " +
                  "     (RowID, " +
                  ...
                  "     SBKodeProduksi, SBPanjang, SBBeratBrutto, SBBeratNetto, SBKodeMedia, SBBeratMedia, SisaDiMesin, KodeKaryawan, KetToleransi, " +
                  "     UserID, TRX) " +
                  "VALUES " +
                  "     (REPLACE(UUID(),'-',''), " +
                  "     @NoTransaksi, @TglTransaksi, @TglPencatatan, @NomorWO, @KodeUnitSAP, @KodeUnit, @KodeShift, @KodeGrup, @KodeMesin, @ShiftSebelumnya, " +
        ...
        Public Function FindFGKodeProduksiByRMKodeProduksi(RMKodeProduksi As String) As ProduksiLaminasi
            Dim SQL As String

            SQL = "SELECT * FROM produksilaminasi " +
                  "WHERE " +
                  "RM1KodeProduksi = '" + RMKodeProduksi + "' OR " +
                  "RM2KodeProduksi = '" + RMKodeProduksi + "' "

            Using DBX As IDbConnection = _DBConnection.Connection
                FindFGKodeProduksiByRMKodeProduksi = DBX.Query(Of ProduksiLaminasi)(SQL).FirstOrDefault
            End Using
        End Function

        Public Function FindQtyHasil(NomorWO As String) As ProduksiLaminasi
            Dim SQL As String

            SQL = "SELECT IFNULL(SUM(FGPanjang),0) AS QtyHasil FROM produksilaminasi " +
                  "WHERE NomorWO ='" + NomorWO + "' AND StatusTransaksi =1"

            Using DBX As IDbConnection = _DBConnection.Connection
                Dim CMD As New MySql.Data.MySqlClient.MySqlCommand(SQL, DBX)
                Dim DR As MySql.Data.MySqlClient.MySqlDataReader = CMD.ExecuteReader()
                Dim Temp As ProduksiLaminasi = Nothing

                If DR.Read Then
                    Temp = New ProduksiLaminasi
        ...
        Public Function GetNomorTransaksi(ByVal Periode As String) As String
            ...
            Using DBX As IDbConnection = _DBConnection.Connection()
                ...
                'Nomor Transaksi
                '------------------------------------------------------------------------------------------
                SQL = "SELECT COUNT(*) AS Total FROM produksilaminasi " +
